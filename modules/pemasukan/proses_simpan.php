...
// jika user sudah login, maka jalankan perintah untuk insert
  ...
  if (isset($_POST['simpan'])) {
    ...
    $jumlah_pemasukan  = str_replace('.', '', $jumlah);
    ...
    // tentukan direktori penyimpanan file bukti transaksi pemasukan
    $path               = "../../images/pemasukan/" . $nama_file_enkripsi;

    // mengecek data file bukti transaksi dari form entri data
    // jika data tidak ada
    if (empty($nama_file)) {
      // sql statement untuk insert data ke tabel "tbl_transaksi"
      $insert = mysqli_query($mysqli, "INSERT INTO tbl_transaksi(tanggal, kategori, deskripsi, pemasukan) 
                                       VALUES('$tanggal_transaksi', '$kategori', '$deskripsi', '$jumlah_pemasukan')")
                                       or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
      // cek query
      // jika proses insert berhasil
      if ($insert) {
        // alihkan ke halaman pemasukan dan tampilkan pesan berhasil simpan data
        header('location: ../../main.php?module=pemasukan&pesan=1');
      }
    }
    // jika data ada
    else {
      ...
      if (move_uploaded_file($tmp_file, $path)) {
        // sql statement untuk insert data ke tabel "tbl_transaksi"
        $insert = mysqli_query($mysqli, "INSERT INTO tbl_transaksi(tanggal, kategori, deskripsi, pemasukan, bukti_transaksi) 
                                         VALUES('$tanggal_transaksi', '$kategori', '$deskripsi', '$jumlah_pemasukan', '$nama_file_enkripsi')")
                                         or die('Ada kesalahan pada query insert : ' . mysqli_error($mysqli));
        // cek query
        // jika proses insert berhasil
        if ($insert) {
